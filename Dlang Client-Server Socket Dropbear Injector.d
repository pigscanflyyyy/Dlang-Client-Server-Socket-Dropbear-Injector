/*
    ...
    How use: injector.exe "127.0.0.1:8088" "GET / HTTP/1.1\r\nhost: www.bing.com\r\n\r\n"
*/
...
void conecta(Socket c, int conn_number, string PAYLOAD){
...
void main(string[] args){
    ...
    try {
        l.bind(new InternetAddress(LISTEN_PORT[0..LISTEN_PORT.indexOf(":")].strip(), to!ushort(LISTEN_PORT[LISTEN_PORT.indexOf(":")+1..LISTEN_PORT.length].strip())));
    } catch(Exception){
    ...
    while(true){
        conn_number += 1;
        task!conecta(l.accept(), conn_number, PAYLOAD).executeInNewThread();
    }
